...
class TestSwitchInterfaceTranslator(util.TestCase):
    ...
    def test_raw_to_event_raises_ValueError_on_unknown_input(self):
        with self.assertRaises(ValueError):
            self.translator.raw_data_to_event(b'\x11')

    def test_raw_to_event_raises_TypeError_on_wrong_input_type(self):
    ...
    def test_event_to_raw_data_raises_ValueError_on_unknown_event_type(self):
        with self.assertRaises(ValueError):
            self.translator.event_to_raw_data('unknown')

    @unittest.skip
    def test_event_to_raw_data_raises_TypeError_when_not_given_and_event(self):
        self.fail('TODO')
